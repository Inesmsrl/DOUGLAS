...
# RR combinés par année pour chaque scénario (implémentation linéaire)
  combined_rr_lin <- import(here("data_clean", "combined_rr_lin.xlsx"))
...
# Sélectionner les MR entre les bornes temporelles du modèle et pivoter le dataframe en format long
  MR_2050 <- MR %>% 
    select("age", "2019":"2050") %>% ## Modifier pour généraliser avec year_i et year_f
    pivot_longer(cols = starts_with("20"), names_to = "year", values_to = "MR") %>% 
...
# Nombre total de décès par année et sur la période 2019-2050 par scénario

     total_deaths_lin <- deaths_evo_lin %>% 
         group_by(scenario) %>%                                 
         summarise(across("2019":"2050", sum)) %>%              # Sum of each column from 2019 to 2050
...
# Nombre de décès évités en 2035 par rapport à 2019, par âge dans chaque scénario
  ...
  graph_avoided_deaths_2035_lin <- ggplot(avoided_2035_lin, aes(x = age,
                                                                y = avoided_2035))+
    facet_wrap(~ scenario, scales = "fixed")+
  ...
  graph_avoided_deaths_2050_lin <- ggplot(avoided_2050_lin, aes(x = age,
                                                                y = avoided_2050))+
    facet_wrap(~ scenario, scales = "fixed")+
    geom_bar(stat = "identity" ,
             fill = "maroon")+
...
# Nombre de décès évités en 2035 dans chaque scénario par rapport au tendanciel
  ...
  graph_avoided_deaths_2035_tend_lin <- ggplot (avoided_deaths_lin, aes(x = age,
                                                                        y = `2035`))+
    facet_wrap(~ scenario, scales = "fixed")+
    geom_bar(stat = "identity" ,
             fill = "cadetblue")+
...
# Nombre de décès évités en 2035 dans chaque scénario par rapport au tendanciel
  ...
  graph_avoided_deaths_2050_tend_lin <- ggplot (avoided_deaths_lin, aes(x = age,
                                                                        y = `2050`))+
    facet_wrap(~ scenario, scales = "fixed")+
    geom_bar(stat = "identity" ,
             fill = "cadetblue")+
...
# Nombre total de décès évités dans chaque scénario par rapport au tendanciel
...
# Décès évités en 2035 par rapport à 2019
    
  # Implémentation linéaire des régimes alimentaires
    export(avoided_2035_lin, here("data_clean", "avoided_2035_2019_lin.xlsx"))
    export(total_avoided_2035_lin, here("data_clean", "total_avoided_2035_2019_lin.xlsx"))
    ggsave(here("results", "avoided_2035_2019_lin.pdf"), plot = graph_avoided_deaths_2035_lin)
    
  # Implémentation par interpolation cosinus des régimes alimentaires
    export(avoided_2035_cos, here("data_clean", "avoided_2035_2019_cos.xlsx"))
    export(total_avoided_2035_cos, here("data_clean", "total_avoided_2035_2019_cos.xlsx"))
    ggsave(here("results", "avoided_2035_2019_cos.pdf"), plot = graph_avoided_deaths_2035_cos)
    
  # Implémentation sigmoïdale des régimes alimentaires
    export(avoided_2035_sig, here("data_clean", "avoided_2035_2019_sig.xlsx"))
    export(total_avoided_2035_sig, here("data_clean", "total_avoided_2035_2019_sig.xlsx"))
    ggsave(here("results", "avoided_2035_2019_sig.pdf"), plot = graph_avoided_deaths_2035_sig)
    
# Décès évités en 2050 par rapport à 2019
    
  # Implémentation linéaire des régimes alimentaires
    export(avoided_2050_lin, here("data_clean", "avoided_2050_2019_lin.xlsx"))
    export(total_avoided_2050_lin, here("data_clean", "total_avoided_2050_2019_lin.xlsx"))
    ggsave(here("results", "avoided_2050_2019_lin.pdf"), plot = graph_avoided_deaths_2050_lin)
    
  # Implémentation par interpolation cosinus des régimes alimentaires
    export(avoided_2050_cos, here("data_clean", "avoided_2050_2019_cos.xlsx"))
    export(total_avoided_2050_cos, here("data_clean", "total_avoided_2050_2019_cos.xlsx"))
    ggsave(here("results", "avoided_2050_2019_cos.pdf"), plot = graph_avoided_deaths_2050_cos)
    
  # Implémentation sigmoïdale des régimes alimentaires
    export(avoided_2050_sig, here("data_clean", "avoided_2050_2019_sig.xlsx"))
    export(total_avoided_2050_sig, here("data_clean", "total_avoided_2050_2019_sig.xlsx"))
    ggsave(here("results", "avoided_2050_2019_sig.pdf"), plot = graph_avoided_deaths_2050_sig)

# Décès évités en 2035 par rapport au tendanciel
    
  # Implémentation linéaire des régimes alimentaires
    ggsave(here("results", "avoided_2035_tend_lin.pdf"), plot = graph_avoided_deaths_2035_tend_lin)
    
  # Implémentation par interpolation cosinus des régimes alimentaires
    ggsave(here("results", "avoided_2035_tend_cos.pdf"), plot = graph_avoided_deaths_2035_tend_cos)
    
  # Implémentation sigmoïdale des régimes alimentaires
    ggsave(here("results", "avoided_2035_tend_sig.pdf"), plot = graph_avoided_deaths_2035_tend_sig)
    
# Décès évités en 2050 par rapport au tendanciel
    
    # Implémentation linéaire des régimes alimentaires
    ggsave(here("results", "avoided_2050_tend_lin.pdf"), plot = graph_avoided_deaths_2050_tend_lin)
    
    # Implémentation par interpolation cosinus des régimes alimentaires
    ggsave(here("results", "avoided_2050_tend_cos.pdf"), plot = graph_avoided_deaths_2050_tend_cos)
    
    # Implémentation sigmoïdale des régimes alimentaires
    ggsave(here("results", "avoided_2050_tend_sig.pdf"), plot = graph_avoided_deaths_2050_tend_sig)
