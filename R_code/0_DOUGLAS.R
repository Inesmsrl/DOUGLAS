...
# S1
graph_rr_fg_combined_sim_sc1 <- ggplot(
...
# S2
graph_rr_fg_combined_sim_sc2 <- ggplot(
  simulations_summary_rr_fg_combined %>%
    filter(scenario == "sc2"),
  aes(
    x = year_n,
...
# S3
graph_rr_fg_combined_sim_sc3 <- ggplot(
  simulations_summary_rr_fg_combined %>%
    filter(scenario == "sc3"),
  aes(
    x = year_n,
...
# S4
graph_rr_fg_combined_sim_sc4 <- ggplot(
  simulations_summary_rr_fg_combined %>%
    filter(scenario == "sc4"),
  aes(
    x = year_n,
...
graph_rr_diets_sim <- ggplot(simulations_summary_rr_diets, aes(
  x = year,
  y = mean_rr,
  color = scenario
)) +
...
graph_rr_diets_relative_sim <- ggplot(
...
graph_total_avoided_deaths_facet <- ggplot(
  simulations_summary_avoided_deaths %>%
    filter(scenario != "actuel"),
  aes(
    x = year,
    y = mean_rr,
...
graph_total_avoided_deaths <- ggplot(
  simulations_summary_avoided_deaths %>%
    filter(scenario != "actuel"),
  aes(
    x = year,
...
# Nombre de décès évités / taille de la population
graph_total_avoided_deaths_rel <- ggplot(
  simulations_summary_avoided_deaths_rel %>%
    filter(scenario != "actuel"),
  aes(
    x = year,
    y = mean_rr,
...
graph_total_avoided_deaths_shift_facet <- ggplot(
  simulations_summary_avoided_deaths_shift %>%
    filter(scenario != "actuel"),
  aes(
    x = year,
    y = mean_rr,
...
graph_total_avoided_deaths_shift <- ggplot(
  simulations_summary_avoided_deaths_shift %>%
    filter(scenario != "actuel"),
  aes(
    x = year,
...
graph_avoided_deaths_dates <- ggplot(
  simulations_summary_avoided_deaths %>%
    filter(
...
#avoided_deaths <- import(here("results", "Main analysis", "avoided_deaths.csv"))
...
export(results, here("results", "Main analysis", "data_deaths.csv"))

# Data set avec nombre de simulations réduit
# results <- results %>%
#   filter(simulation_id %in% sample(simulation_id, 10)) # Sélection de 2 simulations
#
...
deaths_transfer_3 <- function(scen) {
  ...
  for (y in years) {
    for (a in ages) {
      ...
      # Calculer les décès évités pour l'année y et l'âge a
      results <- results %>%
        group_by(simulation_id, year, age) %>%
        mutate(deaths_actuel = first(deaths[scenario == "actuel"])) %>% # Colonne temporaire pour les décès actuels
        ungroup() %>%
      ...
      results <- results %>%
        mutate(population = case_when(
          year == y + 1 & age == a + 1 & age > 18 & age < 105 ~ population + avoided_deaths[year == y & age == a & simulation_id == simulation_id],
          year == y + 1 & age == 105 ~ population + avoided_deaths[year == y & age == 105 & simulation_id == simulation_id],
          TRUE ~ population
        ))
    }
  }
...
deaths_transfer_optimized <- function(scen) {
  # Conversion en data.table
  setDT(results)

  # Filtrer uniquement les scénarios nécessaires
  results <- results[scenario %in% c(scen, "actuel")]

  # Boucle sur les années et les âges
  for (y in years) {
    for (a in ages) {
      # Mise à jour des décès pour l'année y et l'âge a
      results[year == y & age == a, deaths := adjusted_mr * population, by = .(simulation_id, scenario)]

      # Calcul des décès évités
      results[year == y & age == a, deaths_actuel := deaths[scenario == "actuel"], by = .(simulation_id)]
      results[year == y & age == a, avoided_deaths := deaths_actuel - deaths]
      results[, deaths_actuel := NULL] # Suppression de la colonne temporaire

      # Vérification des décès évités pour l'année et l'âge spécifiés
      # avoided_deaths_for_y_a <- results[year == y & age == a, sum(avoided_deaths, na.rm = TRUE)]
      # print(paste("Avoided deaths for year", y, "and age", a, ": ", avoided_deaths_for_y_a))


      # Cas général : transfert des décès évités à (y+1, a+1)
      results[year == y + 1 & age == a + 1 & age < 105,
        population := population + avoided_deaths[year == y & age == a],
        by = .(simulation_id)
      ]

      # Cas spécifique : les 105 ans conservent les décès évités
      results[year == y + 1 & age == 105,
        population := population + avoided_deaths[year == y & age == a],
        by = .(simulation_id)
      ]
    }
...
deaths_sc1_opt <- deaths_transfer_optimized("sc1")
deaths_sc2_opt <- deaths_transfer_optimized("sc2")
...
deaths_transfer_optimized <- function(scen) {
  ...
  for (y in years) {
    for (a in ages) {
      ...
      results[year == y & age == a, deaths_actuel := deaths[scenario == "actuel"], by = .(simulation_id)]
      results[year == y & age == a, avoided_deaths := deaths_actuel - deaths]
      results[, deaths_actuel := NULL] # Nettoyage

      # Transfert des décès évités à (y+1, a+1)
      results[year == y & age == a, `:=`(next_year = y + 1, next_age = a + 1)] # Création de colonnes temporaires
      results[results,
        on = .(simulation_id, year = next_year, age = next_age),
        population := population + i.avoided_deaths
      ]

      # Cas spécifique : 105 ans
      results[year == y & age == 105, next_year := y + 1] # Seulement l'année change, âge reste 105
      results[results,
        on = .(simulation_id, year = next_year, age = 105),
        population := population + i.avoided_deaths
      ]

      # Suppression des colonnes temporaires
      results[, `:=`(next_year = NULL, next_age = NULL)]
    }
  }
...
## Données gérérées avec python

deaths_rep <- import(here("results", "Main analysis", "data_python.csv"))

# Eliminer Les valeurs 5% les plus extrêmes
deaths_rep <- deaths_rep %>%
  group_by(scenario, year, age) %>%
  filter(between(deaths, quantile(deaths, 0.025), quantile(deaths, 0.975)))
...
ggsave(here("results", "visualization_tool_ic95_sim", "total_avoided_deaths.pdf"), plot = graph_tot_av_deaths)
ggsave(here("results", "visualization_tool_ic95_sim", "total_avoided_deaths_dates.pdf"), plot = graph_tot_av_deaths_dates)
